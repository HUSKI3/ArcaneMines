...
const ItemSlotClass = preload("res://scripts/inventory/slot.gd");
...
onready var characterPanel = get_node("../bottom_panel");
...
func _on_AddItemButton_pressed(item):
...
func reset():
  var slots = get_node("SlotsContainer/Slots");
  for slot in slotList:
    print('Gonna remove: ',slot.item)
    slot.item = null
    slots.remove_child(slot.item)
